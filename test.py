...
parser.add_argument("--data_dir", default="/scratch/gilbreth/li2068/UNETR/BTCV/dataset/Dataset300_synrad", type=str, help="dataset directory")
...
def main():
    ...
    if args.saved_checkpoint == "torchscript":
        ...
        # change the next line to normal load procedure if not using distributed during training
        model.load_state_dict({k.replace('module',''):v for k,v in model_dict['state_dict'].items()})
    model.eval()
    model.to(device)

    ''' the below script is used for validation '''
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(val_loader):
            ...
            img = nib.Nifti1Image(val_outputs[0], np.eye(4))  # Save axis for data (just identity)
            img.header.get_xyzt_units()
            img.to_filename(os.path.join('/scratch/gilbreth/li2068/UNETR/BTCV/predicted_results',f'{img_name}'))  # Save as NiBabel file
